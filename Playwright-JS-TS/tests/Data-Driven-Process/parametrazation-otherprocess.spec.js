...
import {LoginPage} from '../POM-Type/pageobject'

import {OrderCreate} from '../POM-Type/pageobject-Productordercreate';


import Dataset from '../Utills/data.json';


import {custombase} from './data-json'
...
for (const data of DataSets) {
    ...
    await createOrderFlow.searchproduct(data.productName)
    ...
    await createOrderFlow.validateproductinCart(data.productName)
...
custombase.only("new order flow for",async({page,testDataForOrder})=>{
    ...
    const createOrderFlow=new OrderCreate(page)
    await createOrderFlow.searchproduct(testDataForOrder.productName)
    await createOrderFlow.goToCart()
    ...
    // const itemincart= page.locator(".cartSection h3");
    // await expect(itemincart).toHaveText(productName);
    await createOrderFlow.validateproductinCart(testDataForOrder.productName)
